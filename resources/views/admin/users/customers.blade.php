    ...
    <div class="bg-[#111111] border border-[#242424] rounded-xl p-4">
        <form method="GET" action="{{ route('admin.users.customers') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="md:col-span-2">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama, email, atau nomor telepon..." class="w-full px-4 py-3 bg-[#000000] border border-[#242424] rounded-xl text-white placeholder-[#64748B] focus:outline-none focus:border-[#B22222] transition-all duration-300">
            </div>
            ...
            <div class="md:col-span-3 flex gap-3">
                ...
                <a href="{{ route('admin.users.customers') }}" class="px-6 py-3 bg-[#242424] text-white rounded-xl font-semibold hover:bg-[#333333] transition-all duration-300">
                    Reset
                </a>
                @endif
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead class="bg-[#0A0A0A] border-b border-[#242424]">
                    <tr>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-[#94A3B8] uppercase tracking-wider">Customer</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-[#94A3B8] uppercase tracking-wider">Nomor Telepon</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-[#94A3B8] uppercase tracking-wider">Total Order</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-[#94A3B8] uppercase tracking-wider">Total Spending</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-[#94A3B8] uppercase tracking-wider">Pembelian Terakhir</th>
                    ...
                    <tr class="hover:bg-[#0A0A0A] transition-all duration-300">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 bg-gradient-to-br from-[#3B82F6] to-[#8B5CF6] rounded-full flex items-center justify-center text-white font-bold">
                                    {{ strtoupper(substr($customer->name, 0, 1)) }}
                                </div>
                                <div>
                                    <div class="text-white font-semibold">{{ $customer->name }}</div>
                                    <div class="text-xs text-[#94A3B8]">{{ $customer->email }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-[#94A3B8]">{{ $customer->phone ?? '-' }}</td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                <svg class="w-4 h-4 text-[#FFD700]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/>
                                </svg>
                                <span class="text-white font-semibold">{{ $customer->orders_count }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="text-[#22C55E] font-semibold">Rp {{ number_format($customer->total_spent ?? 0, 0, ',', '.') }}</div>
                        </td>
                        <td class="px-6 py-4 text-[#94A3B8]">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                <button onclick="openDetailModal({{ json_encode($customer) }})" class="p-2 bg-[#3B82F6]/10 text-[#3B82F6] rounded-lg hover:bg-[#3B82F6]/20 transition-all duration-300" title="Detail">
                                ...
                                <form action="{{ route('admin.users.customers.suspend', $customer) }}" method="POST" onsubmit="return confirm('Yakin ingin suspend customer ini?')">
                                    @csrf
                                    <button type="submit" class="p-2 bg-[#EF4444]/10 text-[#EF4444] rounded-lg hover:bg-[#EF4444]/20 transition-all duration-300" title="Suspend">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"/>
                                        </svg>
                                    </button>
                                ...
                                <form action="{{ route('admin.users.customers.activate', $customer) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="p-2 bg-[#22C55E]/10 text-[#22C55E] rounded-lg hover:bg-[#22C55E]/20 transition-all duration-300" title="Activate">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                        </svg>
                                    </button>
...
<div id="detailModal" class="hidden fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div class="bg-[#111111] border border-[#242424] rounded-2xl max-w-3xl w-full p-6 max-h-[90vh] overflow-y-auto">
        ...
        <div class="space-y-6">
            ...
            <div class="flex items-center gap-4 pb-6 border-b border-[#242424]">
                <div class="w-16 h-16 bg-gradient-to-br from-[#3B82F6] to-[#8B5CF6] rounded-full flex items-center justify-center text-white text-2xl font-bold" id="detail_avatar"></div>
                <div>
                    <div class="text-xl font-bold text-white" id="detail_name"></div>
                    <div class="text-[#94A3B8]" id="detail_email"></div>
                    <div class="text-[#64748B] text-sm mt-1" id="detail_phone"></div>
            ...
            {{-- Statistics --}}
            <div>
                <h4 class="text-sm font-semibold text-[#94A3B8] mb-3">Statistik Pembelian</h4>
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div class="bg-[#0A0A0A] rounded-xl p-4">
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            <span class="text-xs text-[#64748B]">Terakhir</span>
                        </div>
                        <div class="text-sm font-bold text-white" id="detail_last_purchase"></div>
                    </div>
                </div>
            </div>
            ...
            {{-- Order History --}}
            <div>
                <h4 class="text-sm font-semibold text-[#94A3B8] mb-3">Riwayat Pembelian</h4>
                <div class="bg-[#0A0A0A] rounded-xl overflow-hidden">
                    <div id="order_history" class="divide-y divide-[#242424]">
...
function openDetailModal(customer) {
    ...
    if (customer.orders && customer.orders.length > 0) {
        customer.orders.slice(0, 5).forEach(order => {
            ...
            orderHistoryHTML += `
                <div class="p-4 hover:bg-[#111111] transition-all duration-300">
                    <div class="flex items-center justify-between mb-2">
                        <div class="font-semibold text-white">${order.event ? order.event.name : 'Event'}</div>
                        <span class="px-2 py-1 rounded-full text-xs font-semibold" style="background-color: ${statusColor}20; color: ${statusColor}">${statusText}</span>
                    </div>
        ...
        orderHistoryHTML = '<div class="p-6 text-center text-[#64748B]">Belum ada riwayat pembelian</div>';
    }
