...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    ...
    {{-- Total Events --}}
    <div class="bg-gradient-to-br from-[#0B1220] to-[#050B14] rounded-2xl p-6 border border-white/10 hover:border-[#F5C518]/30 transition-all">
        <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-xs text-gray-500 uppercase tracking-wider">Total Event</span>
        </div>
        <h3 class="text-3xl font-bold text-white mb-1">{{ number_format($stats['total_events']) }}</h3>
    ...
    <div class="bg-gradient-to-br from-[#0B1220] to-[#050B14] rounded-2xl p-6 border border-white/10 hover:border-[#F5C518]/30 transition-all">
        <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-xs text-gray-500 uppercase tracking-wider">Total Order</span>
        </div>
        <h3 class="text-3xl font-bold text-white mb-1">{{ number_format($stats['total_orders']) }}</h3>
        <p class="text-sm text-gray-500">Pesanan berhasil</p>
    </div>

    {{-- Total Revenue --}}
    <div class="bg-gradient-to-br from-[#0B1220] to-[#050B14] rounded-2xl p-6 border border-white/10 hover:border-[#F5C518]/30 transition-all">
        <div class="flex items-center justify-between mb-4">
            <div class="w-12 h-12 rounded-xl bg-[#F5C518]/10 flex items-center justify-center">
        ...
        <h3 class="text-3xl font-bold text-[#F5C518] mb-1">Rp {{ number_format($stats['total_revenue'], 0, ',', '.') }}</h3>
    ...
    <div class="bg-gradient-to-br from-[#0B1220] to-[#050B14] rounded-2xl p-6 border border-white/10 hover:border-[#F5C518]/30 transition-all">
        <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-xs text-gray-500 uppercase tracking-wider">Total User</span>
        </div>
        <h3 class="text-3xl font-bold text-white mb-1">{{ number_format($stats['total_users']) }}</h3>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
    ...
    {{-- Latest Orders --}}
    <div class="bg-gradient-to-br from-[#0B1220] to-[#050B14] rounded-2xl border border-white/10">
                    ...
                    @foreach($latestOrders as $order)
                        <div class="flex items-center gap-4 p-4 rounded-xl bg-white/5 hover:bg-white/10 transition-all">
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-semibold text-white truncate">{{ $order->event->title }}</p>
                                <p class="text-xs text-gray-500 mt-0.5">{{ $order->user->name }} • {{ $order->quantity }} tiket</p>
                            </div>
                            <div class="text-right flex-shrink-0">
                                <p class="text-sm font-bold text-[#F5C518]">Rp {{ number_format($order->total_price, 0, ',', '.') }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
    ...
    <div class="bg-gradient-to-br from-[#0B1220] to-[#050B14] rounded-2xl border border-white/10">
        <div class="p-6 border-b border-white/10">
            <h2 class="text-lg font-bold text-white">Event Terdekat</h2>
        </div>
                ...
                <div class="space-y-4">
                    @foreach($upcomingEvents as $event)
                        <div class="flex items-center gap-4 p-4 rounded-xl bg-white/5 hover:bg-white/10 transition-all">
                            ...
                            <div class="text-right flex-shrink-0">
                                <p class="text-sm font-bold text-[#F5C518]">
                                    @if($event->price == 0)
                                        GRATIS
                                    @else
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    ...
    <a href="{{ route('admin.banners.create') }}" 
       class="bg-gradient-to-br from-[#0B1220] to-[#050B14] rounded-2xl p-6 border border-white/10 hover:border-[#F5C518]/50 transition-all group">
