...
<div class="mb-8">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-white tracking-wider mb-2">Edit Event</h1>
            <p class="text-gray-500 text-sm">Perbarui informasi event</p>
        </div>
...
<form action="{{ route('admin.events.update', $event) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="bg-[#0B1220] rounded-2xl border border-white/10 p-8 space-y-6">
        ...
        <div class="space-y-5">
            ...
            <div>
                ...
                <textarea name="description" 
                          rows="5"
                          class="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-gray-600 focus:outline-none focus:border-[#F5C518] focus:ring-1 focus:ring-[#F5C518] transition-all resize-none">{{ old('description', $event->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="number" 
                           ...
                           min="1"
                           class="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-gray-600 focus:outline-none focus:border-[#F5C518] focus:ring-1 focus:ring-[#F5C518] transition-all">
                    <p class="text-xs text-gray-600 mt-1">Saat ini: {{ $event->remaining_quota }} tersisa dari {{ $event->quota }}</p>
                    @error('quota')
                        <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="space-y-5">
            <h3 class="text-lg font-bold text-[#F5C518] tracking-wider">TAMPILAN DI HOMEPAGE</h3>
            <p class="text-sm text-gray-400">Pilih section mana saja event ini akan ditampilkan</p>
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                {{-- Rekomendasi Event --}}
                <label class="flex items-center gap-3 p-4 rounded-xl bg-white/5 border border-white/10 hover:border-[#F5C518]/50 cursor-pointer transition-all">
                    <input type="checkbox" 
                           name="show_in_recommended" 
                           value="1"
                           {{ old('show_in_recommended', $event->show_in_recommended ?? false) ? 'checked' : '' }}
                           class="w-5 h-5 rounded border-gray-600 text-[#F5C518] focus:ring-[#F5C518] focus:ring-offset-gray-800">
                    <div>
                        <p class="text-white font-semibold">· Rekomendasi Event</p>
                        <p class="text-xs text-gray-500">Tampil di section "Rekomendasi Event"</p>
                    </div>
                </label>
                ...
                <label class="flex items-center gap-3 p-4 rounded-xl bg-white/5 border border-white/10 hover:border-[#F5C518]/50 cursor-pointer transition-all">
                    <input type="checkbox" 
                           ...
                           value="1"
                           {{ old('show_in_nearest', $event->show_in_nearest ?? false) ? 'checked' : '' }}
                           class="w-5 h-5 rounded border-gray-600 text-[#F5C518] focus:ring-[#F5C518] focus:ring-offset-gray-800">
                    <div>
                        <p class="text-white font-semibold">📍 Event Terdekat</p>
                        <p class="text-xs text-gray-500">Tampil di section "Event Terdekat"</p>
                    </div>
                </label>
                ...
                <label class="flex items-center gap-3 p-4 rounded-xl bg-white/5 border border-white/10 hover:border-[#F5C518]/50 cursor-pointer transition-all">
                    <input type="checkbox" 
                           ...
                           value="1"
                           {{ old('show_in_upcoming', $event->show_in_upcoming ?? false) ? 'checked' : '' }}
                           class="w-5 h-5 rounded border-gray-600 text-[#F5C518] focus:ring-[#F5C518] focus:ring-offset-gray-800">
                    <div>
                        <p class="text-white font-semibold">🗓️ Upcoming Event</p>
                        <p class="text-xs text-gray-500">Tampil di section "Upcoming Event"</p>
                    </div>
                </label>
                ...
                <label class="flex items-center gap-3 p-4 rounded-xl bg-white/5 border border-white/10 hover:border-[#F5C518]/50 cursor-pointer transition-all">
                    <input type="checkbox" 
                           ...
                           value="1"
                           {{ old('show_in_popular', $event->show_in_popular ?? false) ? 'checked' : '' }}
                           class="w-5 h-5 rounded border-gray-600 text-[#F5C518] focus:ring-[#F5C518] focus:ring-offset-gray-800">
                    <div>
                        <p class="text-white font-semibold">🔥 Popular Event</p>
                        <p class="text-xs text-gray-500">Tampil di section "Popular Event"</p>
                    </div>
                </label>
            </div>
        </div>
        ...
        <div class="space-y-5">
                ...
                @if($event->image)
                    <div class="mb-4">
                        ...
                        <img src="{{ asset('storage/' . $event->image) }}" 
                             ...
                             class="w-48 h-48 object-cover rounded-xl border border-white/10">
                    </div>
                @endif
            </div>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">
                    Ganti Gambar <span class="text-gray-500">(Optional - Kosongkan jika tidak ingin mengganti)</span>
                </label>
...
function previewImage() {
    ...
    if (file) {
        ...
        if (sizeInMB > 2) {
            ...
            fileSizeDisplay.classList.remove('text-gray-600', 'text-green-400');
